...
library(ggplot2)
    ...
    percentile <- apply(storage.df, 2 , quantile , probs = quants , na.rm = TRUE )
    
    plot(1:12,percentile[1,1:12],type="l",xlim=range(1:12),ylim=range(0:11),xaxs="i",yaxs="i",xlab="Month",ylab="Storage (BG)",axes=FALSE,frame.plot=TRUE)
    ...
    polygon(c(1:12,12:1),c(percentile[1,],rev(percentile[2,])),col="indianred1",border=NA)
    polygon(c(1:12,12:1),c(percentile[2,],rev(percentile[3,])),col="lightpink",border=NA)
    polygon(c(1:12,12:1),c(percentile[3,],rev(percentile[4,])),col="rosybrown1",border=NA)
    polygon(c(1:12,12:1),c(percentile[4,],rev(percentile[5,])),col="moccasin",border=NA)
    polygon(c(1:12,12:1),c(percentile[5,],rev(percentile[6,])),col="lightgreen",border=NA)
    polygon(c(1:12,12:1),c(percentile[6,],rev(percentile[7,])),col="seagreen3",border=NA)
    
    #polygon(c(1:12,12:1),c(percentile[1,],rev(percentile[3,])),col="skyblue",border=NA)
    lines(1:12,(c(rep(capacity*0.9,12))),lty=2,lwd=1, cex=1.2, col="black")
    #lines(1:12,(percentile[4,1:12]),lty=1,lwd=2,col="orange", type="b", pch=18, cex=2)
    
    lines(1:12,(percentile[4,1:12]),lty=1,lwd=2,col="yellow", type="b", pch=15, cex=1.5)
    lines(1:12,(percentile[7,1:12]),lty=1,lwd=2,col="springgreen4", type="b", pch=23, cex=1.5)
    lines(1:12,(percentile[1,1:12]),lty=1,lwd=2, type="b", pch=19, cex=1.2, col="red")
